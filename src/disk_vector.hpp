...
public:
  ...
  void dmat_columns_sum(DiskMatrixT &dmat);
...
/* Divide each term of this by the term of ldvec with the same index and accumulate values of this when the division is undefined : 
 * If ldvec[i] != 0 then this[i]/=ldvec[i]
 * If ldvec[i] == 0 then aggregator+=this[i] ; this[i]=0;
 */
...
template <typename VectorT>
uint64_t DiskVector<VectorT>::get_n_slices(){
...
template <typename VectorT>
uint64_t DiskVector<VectorT>::countZeros(){
uint64_t res = 0;
...
template <typename VectorT>
void DiskVector<VectorT>::divide_and_sum_Nan(DiskVector<VectorT> &ldvec, float& aggregator)
{
 if (props->nslices != ldvec.props->nslices)
  {
    std::ostringstream oss;
    oss << "Wrong dimensions in Hadamard division, both vector must have the same size";
    print_error(oss.str());
    exit(-1);
  }
