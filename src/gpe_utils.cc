...
void gpe_log (std::string message) {
  ...
  std::cout << result->tm_hour << ":" << result->tm_min << ":" << result->tm_sec
    << "\033[1m[GRAPHEE] [LOG] \033[0m " << message << std::endl;
}

void gpe_warning (std::string message) {
  std::time_t result = std::time(nullptr);
  std::cout << result->tm_hour << ":" << result->tm_min << ":" << result->tm_sec
    << "\033[1;36m[GRAPHEE] [WARN]\033[0m \033[36m" << message << "\033[0m" << std::endl;
}

void gpe_error (std::string message) {
  std::time_t result = std::time(nullptr);
  std::cout << result->tm_hour << ":" << result->tm_min << ":" << result->tm_sec
    << "\033[1;31m[GRAPHEE] [ERR] \033[0m \033[31m" << message << "\033[0m" << std::endl;
}
